...
\section{Test Methodology}
  ...
  Through parameterization this complexity is abstracted. The test code only uses the parameters and the values of the parameters are easily manipulated depending on UI changes and or test configurations.
	...
	\begin{itemize}
		\item the method takes a single argument: a JSON parameter with a predefined string of delimited tags. For example \textit{tagTest1 = "air,cmd,sensor,temp"}; there 14 similar varieties of tag tests. These are infinately flexible, simply a different string can be passed in for a different set of tags.
	...
	Building Operator is designed so that the building operator can control a number of geo-located sites. 
